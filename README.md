# ItaloMQ - zeromq-based message passing for Italo projects

This C++17 library contains an abstraction layer around ZeroMQ to support integration with Italo
...
and local daemon commands, communication channels between italod and auxiliary services (storage
server, italonet), and also provides a local multithreaded job scheduling within a process.
...
ItaloMQ messages come in two fundamental forms: "commands", consisting of a command named and
...
either direction at any time.  ItaloMQ sets up a "listener" and "client" connections, but these only
...
`category.command` - for commands/requests registered by the ItaloMQ caller (e.g. italod).  Here
...
on this below).  For example, for italod categories are:
...
the request via ItaloMQ you provide a callback to be invoked when the reply arrives.  On the wire
this looks like:
...
by ItaloMQ (both the invoker and the recipient code only see the `hello world`/`dlrow olleh` message
parts).
...
Optional command/request arguments are always strings on the wire.  The ItaloMQ-using developer is
...
Internal commands passed between ItaloMQ-managed threads use either plain strings or bt-encoded
dictionaries.  See `italomq/bt_serialize.h` if you want a bt serializer/deserializer.
...
- a string containing a service node pubkey.  In this mode ItaloMQ will look for the given SN in
  already-established connections, reusing a connection if one exists.  If no connection already
  exists, a new connection to the given SN is attempted (this requires constructing the ItaloMQ
...
The callbacks are passed a ItaloMQ::Message object from which the message (plus various connection
...
connection to deliver the message.  In order for this to work the ItaloMQ caller must provide a
lookup function to retrieve the remote address given a SN x25519 pubkey.

### Callbacks

Invoked command functions are always invoked with exactly one arguments: a non-const ItaloMQ::Message
reference from which the connection info, ItaloMQ object, and message data can be obtained.
...
also allows configuration of how public connections should be treated: for example, a italod running
...
For example, in italod the categories described above have authentication levels of:
...
In order to handle ServiceNode authentication, ItaloMQ uses an Allow callback invoked during
...
ItaloMQ operates a pool of worker threads to handle jobs.  The simplest use just allocates new jobs
...
As mentioned above, ItaloMQ tries to avoid exceeding the configured general threads value (G)
...
Instead ItaloMQ provides a mechanism for this by allowing you to submit a batch of jobs with a
...
void continue_big_task(std::vector<italomq::job_result<double>> results) {
    double sum = 0;
    for (auto& r : results) {
        try {
            sum += r.get();
...
void start_big_task() {
    ...
    italomq::Batch<double /*return type*/> batch;
...
exhausting the worker threads with waiting jobs) then you can implement it yourself; ItaloMQ isn't
...
ItaloMQ supports scheduling periodic tasks via the `add_timer()` function.  These timers have an
