...
void ItaloMQ::proxy_send(bt_dict_consumer data) {
...
void ItaloMQ::proxy_reply(bt_dict_consumer data) {
    bool have_conn_id = false;
...
void ItaloMQ::proxy_control_message(std::vector<zmq::message_t>& parts) {
    // We throw an uncaught exception here because we only generate control messages internally in
    // italomq code: if one of these condition fail it's a italomq bug.
    if (parts.size() < 2)
        throw std::logic_error("ItaloMQ bug: Expected 2-3 message parts for a proxy control message");
    ...
    throw std::runtime_error("ItaloMQ bug: Proxy received invalid control command: " +
            std::string{cmd} + " (" + std::to_string(parts.size()) + ")");
}

void ItaloMQ::proxy_loop() {
    ...
    for (size_t i = 0; i < bind.size(); i++) {
        ...
        LMQ_LOG(info, "ItaloMQ listening on ", bind[i].first);
...
// reason)
bool ItaloMQ::proxy_handle_builtin(size_t conn_index, std::vector<zmq::message_t>& parts) {
    ...
    if (cmd == "REPLY") {
        ...
        if (parts.size() == (1 + incoming) && cmd == "UNKNOWNCOMMAND") {
            ...
            LMQ_LOG(debug, "Received plain UNKNOWNCOMMAND; remote is probably an older italomq. Ignoring.");
            return true;
        }
...
void ItaloMQ::proxy_process_queue() {
