...
void ItaloMQ::rebuild_pollitems() {
    pollitems.clear();
...
void ItaloMQ::setup_external_socket(zmq::socket_t& socket) {
...
void ItaloMQ::setup_outgoing_socket(zmq::socket_t& socket, std::string_view remote_pubkey) {
...
ConnectionID ItaloMQ::connect_sn(std::string_view pubkey, std::chrono::milliseconds keep_alive, std::string_view hint) {
...
ConnectionID ItaloMQ::connect_remote(const address& remote, ConnectSuccess on_connect, ConnectFailure on_failure,
...
ConnectionID ItaloMQ::connect_remote(std::string_view remote, ConnectSuccess on_connect, ConnectFailure on_failure,
...
void ItaloMQ::disconnect(ConnectionID id, std::chrono::milliseconds linger) {
...
ItaloMQ::proxy_connect_sn(std::string_view remote, std::string_view connect_hint, bool optional, bool incoming_only, bool outgoing_only, std::chrono::milliseconds keep_alive) {
...
std::pair<zmq::socket_t *, std::string> ItaloMQ::proxy_connect_sn(bt_dict_consumer data) {
...
void ItaloMQ::proxy_close_connection(size_t index, std::chrono::milliseconds linger) {
...
void ItaloMQ::proxy_expire_idle_peers() {
...
void ItaloMQ::proxy_conn_cleanup() {
...
void ItaloMQ::proxy_connect_remote(bt_dict_consumer data) {
...
void ItaloMQ::proxy_disconnect(bt_dict_consumer data) {
...
void ItaloMQ::proxy_disconnect(ConnectionID conn, std::chrono::milliseconds linger) {
