...
void ItaloMQ::set_zmq_context_option(zmq::ctxopt option, int value) {
    context.set(option, value);
}

void ItaloMQ::log_level(LogLevel level) {
    log_lvl.store(level, std::memory_order_relaxed);
}

LogLevel ItaloMQ::log_level() const {
    return log_lvl.load(std::memory_order_relaxed);
}


CatHelper ItaloMQ::add_category(std::string name, Access access_level, unsigned int reserved_threads, int max_queue) {
...
void ItaloMQ::add_command(const std::string& category, std::string name, CommandCallback callback) {
...
void ItaloMQ::add_request_command(const std::string& category, std::string name, CommandCallback callback) {
    add_command(category, name, std::move(callback));
    categories.at(category).commands.at(name).second = true;
}

void ItaloMQ::add_command_alias(std::string from, std::string to) {
...
zmq::socket_t& ItaloMQ::get_control_socket() {
    assert(proxy_thread.joinable());

    // Maps the ItaloMQ unique ID to a local thread command socket.
    ...
    // Optimize by caching the last value; ItaloMQ is often a singleton and in that case we're
    ...
    if (proxy_shutting_down)
        throw std::runtime_error("Unable to obtain ItaloMQ control socket: proxy thread is shutting down");
        ...
        sn_lookup{std::move(lookup)}, log_lvl{level}, logger{std::move(logger)}
{
    ...
    LMQ_TRACE("Constructing ItaloMQ, id=", object_id, ", this=", this);

    if (sodium_init() == -1)
        throw std::runtime_error{"libsodium initialization failed"};

    if (pubkey.empty() != privkey.empty()) {
        throw std::invalid_argument("ItaloMQ construction failed: one (and only one) of pubkey/privkey is empty. Both must be specified, or both empty to generate a key.");
    } else if (pubkey.empty()) {
        if (service_node)
            throw std::invalid_argument("Cannot construct a service node mode ItaloMQ without a keypair");
        LMQ_LOG(debug, "generating x25519 keypair for remote-only ItaloMQ instance");
        ...
        if (verify_pubkey != pubkey)
            throw std::invalid_argument("Invalid pubkey/privkey values given to ItaloMQ construction: pubkey verification failed");
    }
}
...
void ItaloMQ::start() {
    ...
    LMQ_LOG(info, "Initializing ItaloMQ ", bind.empty() ? "remote-only" : "listener", " with pubkey ", to_hex(pubkey));
    ...
    if (MAX_SOCKETS > 1 && MAX_SOCKETS <= zmq_socket_limit)
        ...
        LMQ_LOG(error, "Not applying ItaloMQ::MAX_SOCKETS setting: ", MAX_SOCKETS, " must be in [1, ", zmq_socket_limit, "]");
    ...
    proxy_thread = std::thread{&ItaloMQ::proxy_loop, this};
    ...
    catch (const zmq::error_t &e) { throw std::runtime_error("Failure reading from ItaloMQ::Proxy thread: "s + e.what()); }
...
void ItaloMQ::listen_curve(std::string bind_addr, AllowFunc allow_connection) {
...
void ItaloMQ::listen_plain(std::string bind_addr, AllowFunc allow_connection) {
    // TODO: As above.
    check_not_started(proxy_thread, "start listening");
...
std::pair<ItaloMQ::category*, const std::pair<ItaloMQ::CommandCallback, bool>*> ItaloMQ::get_command(std::string& command) {
...
void ItaloMQ::set_batch_threads(int threads) {
    if (proxy_thread.joinable())
...
void ItaloMQ::set_reply_threads(int threads) {
    if (proxy_thread.joinable())
...
void ItaloMQ::set_general_threads(int threads) {
    if (proxy_thread.joinable())
...
ItaloMQ::run_info& ItaloMQ::run_info::load(category* cat_, std::string command_, ConnectionID conn_, Access access_, std::string remote_,
...
ItaloMQ::run_info& ItaloMQ::run_info::load(category* cat_, std::string command_, std::string remote_, std::function<void()> callback) {
    reset();
    is_injected = true;
    cat = cat_;
    command = std::move(command_);
...
ItaloMQ::run_info& ItaloMQ::run_info::load(pending_command&& pending) {
...
ItaloMQ::run_info& ItaloMQ::run_info::load(batch_job&& bj, bool reply_job, int tagged_thread) {
    reset();
    is_batch_job = true;
...
ItaloMQ::~ItaloMQ() {
    ...
    LMQ_LOG(info, "ItaloMQ shutting down proxy thread");
    detail::send_control(get_control_socket(), "QUIT");
    proxy_thread.join();
    LMQ_LOG(info, "ItaloMQ proxy thread has stopped");
}
