...
bool worker_wait_for(ItaloMQ& lmq, zmq::socket_t& sock, std::vector<zmq::message_t>& parts, const std::string_view worker_id, const std::string_view expect) {
    while (true) {
...
void ItaloMQ::worker_thread(unsigned int index, std::optional<std::string> tagged, std::function<void()> start) {
    ...
    if (tagged) {
        // If we're a tagged worker then we got started up before ItaloMQ started, so we need to wait
        // for an all-clear signal from ItaloMQ first, then we fire our `start` callback, then we can
...
ItaloMQ::run_info& ItaloMQ::get_idle_worker() {
...
void ItaloMQ::proxy_worker_message(std::vector<zmq::message_t>& parts) {
...
void ItaloMQ::proxy_run_worker(run_info& run) {
...
void ItaloMQ::proxy_to_worker(size_t conn_index, std::vector<zmq::message_t>& parts) {
...
void ItaloMQ::inject_task(const std::string& category, std::string command, std::string remote, std::function<void()> callback) {
    if (!callback) return;
...
void ItaloMQ::proxy_inject_task(injected_task task) {
