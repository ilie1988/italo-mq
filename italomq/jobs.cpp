...
void ItaloMQ::proxy_batch(detail::Batch* batch) {
...
void ItaloMQ::job(std::function<void()> f, std::optional<TaggedThreadID> thread) {
...
void ItaloMQ::proxy_schedule_reply_job(std::function<void()> f) {
    auto* b = new Batch<void>;
...
void ItaloMQ::proxy_run_batch_jobs(std::queue<batch_job>& jobs, const int reserved, int& active, bool reply) {
...
void ItaloMQ::proxy_timer(std::function<void()> job, std::chrono::milliseconds interval, bool squelch, int thread) {
    if (!timers)
        timers.reset(zmq_timers_new());

    int timer_id = zmq_timers_add(timers.get(),
            interval.count(),
            [](int timer_id, void* self) { static_cast<ItaloMQ*>(self)->_queue_timer_job(timer_id); },
            this);
    if (timer_id == -1)
        throw zmq::error_t{};
...
void ItaloMQ::proxy_timer(bt_list_consumer timer_data) {
...
void ItaloMQ::_queue_timer_job(int timer_id) {
...
void ItaloMQ::add_timer(std::function<void()> job, std::chrono::milliseconds interval, bool squelch, std::optional<TaggedThreadID> thread) {
...
void ItaloMQ::TimersDeleter::operator()(void* timers) { zmq_timers_destroy(&timers); }

TaggedThreadID ItaloMQ::add_tagged_thread(std::string name, std::function<void()> start) {
    if (proxy_thread.joinable())
    ...
    run.worker_thread = std::thread{&ItaloMQ::worker_thread, this, run.worker_id, name, std::move(start)};
