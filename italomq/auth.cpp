...
bool ItaloMQ::proxy_check_auth(size_t conn_index, bool outgoing, const peer_info& peer,
    ...
    if (!cat_call.first) {
        ...
        LMQ_LOG(warn, "Access denied to ", command, " for peer [", to_hex(peer.pubkey), "]/", peer_address(cmd),
                ": that command is only available when this ItaloMQ is running in service node mode");
...
void ItaloMQ::set_active_sns(pubkey_set pubkeys) {
...
void ItaloMQ::proxy_set_active_sns(std::string_view data) {
    proxy_set_active_sns(detail::deserialize_object<pubkey_set>(bt_deserialize<uintptr_t>(data)));
}
void ItaloMQ::proxy_set_active_sns(pubkey_set pubkeys) {
...
void ItaloMQ::update_active_sns(pubkey_set added, pubkey_set removed) {
...
void ItaloMQ::proxy_update_active_sns(bt_list_consumer data) {
...
void ItaloMQ::proxy_update_active_sns(pubkey_set added, pubkey_set removed) {
...
void ItaloMQ::proxy_update_active_sns_clean(pubkey_set added, pubkey_set removed) {
...
void ItaloMQ::process_zap_requests() {
