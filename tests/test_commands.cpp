...
TEST_CASE("basic commands", "[commands]") {
    ...
    ItaloMQ client{get_logger("C» "), LogLevel::trace};
...
TEST_CASE("deferred replies on incoming connections", "[commands][hey google]") {
    ...
    server.add_request_command("hey google", "remember", [&](Message& m) {
            ...
            if (backdoor)
                m.italomq.send(backdoor, "backdoor.data", m.data[0]);
    });
    ...
    std::vector<std::unique_ptr<ItaloMQ>> clients;
    ...
    for (int i = 0; i < 5; i++) {
        clients.push_back(std::make_unique<ItaloMQ>(
...
TEST_CASE("data parts", "[send][data_parts]") {
    ...
    client.send(c, "public.hello", italomq::send_option::data_parts(some_data.begin(), some_data.end()));
    reply_sleep();
    {
        auto lock = catch_lock();
        std::lock_guard l{mut};
    ...
    client.send(c, "public.hello",
            "hi",
            italomq::send_option::data_parts(some_data2.begin(), some_data2.end()),
            "another",
            "string"sv,
            italomq::send_option::data_parts(some_data.begin(), some_data.end()));
