...
TEST_CASE("plain-text connections", "[plaintext][connect]") {
    std::string listen = random_localhost();
    ItaloMQ server{get_logger("S» "), LogLevel::trace};
...
TEST_CASE("unique connection IDs", "[connect][id]") {
    ...
    ItaloMQ client1{get_logger("C1» "), LogLevel::trace};
    ItaloMQ client2{get_logger("C2» "), LogLevel::trace};
    client1.start();
    client2.start();
...
TEST_CASE("SN disconnections", "[connect][disconnect]") {
    std::vector<std::unique_ptr<ItaloMQ>> lmq;
    ...
    for (int i = 0; i < pubkey.size(); i++) {
        lmq.push_back(std::make_unique<ItaloMQ>(
...
TEST_CASE("SN single worker test", "[connect][worker]") {
    ...
    ItaloMQ client{get_logger("B» "), LogLevel::trace};
    client.start();
