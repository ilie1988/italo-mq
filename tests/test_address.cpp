...
TEST_CASE("tcp QR-code friendly addresses", "[address][tcp][qr]") {
    address a{"tcp://public.italo.foundation:12345"};
    address a_qr{"TCP://PUBLIC.ITALO.FOUNDATION:12345"};
    address b{"tcp://PUBLIC.ITALO.FOUNDATION:12345"};
    REQUIRE( a == a_qr );
    REQUIRE( a != b );
    REQUIRE( a.host == "public.italo.foundation" );
    REQUIRE( a.qr_address() == "TCP://PUBLIC.ITALO.FOUNDATION:12345" );

    address c = address::tcp_curve("public.italo.foundation", 12345, pk);
    REQUIRE( c.qr_address() == "CURVE://PUBLIC.ITALO.FOUNDATION:12345/" + pk_B32Z );
    REQUIRE( address{"CURVE://PUBLIC.ITALO.FOUNDATION:12345/" + pk_B32Z} == c );
    // We don't produce with upper-case hex, but we accept it:
    REQUIRE( address{"CURVE://PUBLIC.ITALO.FOUNDATION:12345/" + pk_HEX} == c );

    // lower case not permitted:                          ▾
    REQUIRE_THROWS_AS(address{"CURVE://PUBLIC.ITALO.FOUNDATiON:12345/" + pk_B32Z}, std::invalid_argument);
    // also only accept upper-base base32z and hex:
    REQUIRE_THROWS_AS(address{"CURVE://PUBLIC.ITALO.FOUNDATION:12345/" + pk_b32z}, std::invalid_argument);
    REQUIRE_THROWS_AS(address{"CURVE://PUBLIC.ITALO.FOUNDATION:12345/" + pk_hex}, std::invalid_argument);
    // don't accept base64 even if it's upper-case (because case-converting it changes the value)
    REQUIRE_THROWS_AS(address{"CURVE://PUBLIC.ITALO.FOUNDATION:12345/AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"}, std::invalid_argument);
    REQUIRE_THROWS_AS(address{"CURVE://PUBLIC.ITALO.FOUNDATION:12345/AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA="}, std::invalid_argument);
}
